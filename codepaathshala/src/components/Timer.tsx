...
export default function Timer({ totalTime, timeOverCallback }: { totalTime?: number, timeOverCallback: () => void }) {
    ...
    useEffect(() => {
        let storedEndTime = sessionStorage.getItem("endTime");
        let storedSectionTime = sessionStorage.getItem("endTimeSection")
        if (storedEndTime) {
            if (storedEndTime && storedEndTime.length > 0) {
                setAssessmentEndTime(new Date(JSON.parse(storedEndTime)));
                return;
            }
    ...
    function setAssessmentEndTime(date: Date) {
        sessionStorage.setItem("endTime", JSON.stringify(date));
        setEndTime(date);
    ...
    return <>
        <div className="rounded-lg flex gap-1 items-center py-1 px-2 border border-green-300 bg-green-100 w-28">
